...
export async function searchRecipesByIngredients(
  ...
  if (!apiKey || apiKey === "YOUR_API_KEY_HERE") {
    throw new Error(
      "Spoonacular API key is missing. Set VITE_SPOONACULAR_API_KEY in your .env file."
    );
  }
...
export async function fetchRecipeNutritionBulk(
  ids: number[]
  ...
  for (const item of data) {
    const proteinNutrient = item.nutrition?.nutrients?.find(
      (n) => n.name === "Protein"
    );
